...
fit <- locfit((fert_use == "Yes")~log(yield),data=all2013[  all2013$yield>0 & !is.na(all2013$yield),])
...
fit <- locfit(ext~log(yield),data=all2013[  all2013$yield>0 & !is.na(all2013$yield),])
fit0 <- scb(fit, ev = lfgrid(100))
### this is an extremely inelegant solution - I can not extract the confidence bands from the scb object, I can only get them when I print the object to the screen.  So I decided to print the to disk and then load then again
sink(file="temp.txt")
print(fit0)
...
fit <- locfit((fert_use == "Yes")*ext~log(yield),data=all2013[  all2013$yield>0 & !is.na(all2013$yield),])
fit0 <- scb(fit, ev = lfgrid(100))
### this is an extremely inelegant solution - I can not extract the confidence bands from the scb object, I can only get them when I print the object to the screen.  So I decided to print the to disk and then load then again
sink(file="temp.txt")
print(fit0)
